...
release, version 4.0.4, is a huge step forward.
Almost all methods are implemented in a highly object-oriented manner (using S4
classes) and they have been written internally in \texttt{C} or call call
...
\subfigure[Local suppression without Argus (functionality is not available)]{
\includegraphics[width=0.46\textwidth]{imgs/compTime2b}
\label{compTime2}}
\caption[]{\label{compTime}Computation time of IHSN C++ code, $\mu$-Argus and
sdcMicro (version 4.0.4).\textcolor{red}{TODO: include computation time of old
sdcMicro and mu-Argus!}}
\end{figure}
